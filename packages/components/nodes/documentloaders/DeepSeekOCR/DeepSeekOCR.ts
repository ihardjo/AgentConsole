...
class DeepSeekOCRAPI_DocumentLoaders implements INode {
    label: string
    name: string
    version: number
    description: string
    ...
    constructor() {
        this.label = 'DeepSeek OCR API Loader'
        this.name = 'deepseekocrapiLoader'
        this.version = 1.0
        this.type = 'Document'
        this.icon = 'pdf.svg'
        ...
        this.inputs = [
            ...
            },
            {
                ...
                type: 'file',
                fileType: '.pdf',
                description: 'Upload a PDF file to send with POST request'
            },
            {
                label: 'Custom Prompt',
                name: 'prompt',
                type: 'string',
                description: 'Optional prompt or instruction for OCR or API request',
                optional: true,
                additionalParams: true
            },
            {
    ...
    async init(nodeData: INodeData, _: string, options: ICommonObject): Promise<any> {
        ...
        // Handle PDF upload and prompt if provided
        if (pdfFileBase64) {
            ...
            if (pdfFileBase64.startsWith('FILE-STORAGE::')) {
                let file = pdfFileBase64.replace('FILE-STORAGE::', '')
                file = file.replace('[', '')
                file = file.replace(']', '')
                const orgId = options.orgId
                const chatflowid = options.chatflowid
                const fileData = await getFileFromStorage(file, orgId, chatflowid)
                bf = Buffer.from(fileData)
                filename = file
            } else {
            ...
            apiLoaderParam.formData = form
            apiLoaderParam.headers = { ...apiLoaderParam.headers, ...form.getHeaders() }
        }
...
class ApiLoader extends BaseDocumentLoader {
    ...
    constructor({ url, headers, body, method, ca, formData }: ApiLoaderParams) {
        super()
        this.url = url
    ...
    public async load(): Promise<IDocument[]> {
        if (this.method === 'POST') {
            return this.executePostRequest(this.url, this.headers, this.body, this.ca, this.formData)
        } else {
    ...
    protected async executePostRequest(
        url: string,
        headers?: ICommonObject,
        body?: ICommonObject,
        ca?: string,
        ...
        try {
            ...
            // If API already returns [{ pageContent, metadata }]
            if (Array.isArray(resData) && resData.every((item) => 'pageContent' in item)) {
                return resData.map(
                    (item: any) =>
                        new Document({
                            pageContent: item.pageContent ?? '',
                            metadata: item.metadata ?? { url }
                        })
                )
            }

            // Fallback to generic JSON -> string document
            const responseJsonString = JSON.stringify(resData, null, 2)
            const doc = new Document({
                pageContent: responseJsonString,
                metadata: {
                    url
...
module.exports = {
    nodeClass: DeepSeekOCRAPI_DocumentLoaders
}
