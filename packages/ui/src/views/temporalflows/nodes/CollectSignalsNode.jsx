...
const NODE_COLOR = '#2196F3' // Blue for Collect Signals

// ===========================|| COLLECT SIGNALS NODE ||=========================== //

const CollectSignalsNode = ({ data }) => {
        ...
        <div onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
            ...
            >
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                    ...
                    >
                        <IconInbox size={24} color='white' />
                    </Box>
                    <Box>
                        <Typography sx={{ fontSize: '0.85rem', fontWeight: 500 }}>{data.label || 'Collect Signals'}</Typography>
                        <Typography sx={{ fontSize: '0.7rem', color: 'text.secondary' }}>{data.signalName || 'No signal name'}</Typography>
                        <Typography sx={{ fontSize: '0.65rem', color: 'text.secondary' }}>
                            {data.requiredCount || 2} signals required
                        </Typography>
                        {data.timeout && (
...
CollectSignalsNode.propTypes = {
    data: PropTypes.object
}

export default memo(CollectSignalsNode)
