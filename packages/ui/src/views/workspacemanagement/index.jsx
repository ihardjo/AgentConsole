...
import AddEditWorkspaceDialog from '@/views/workspacemanagement/AddEditWorkspaceDialog'
import EditWorkspaceUserRoleDialog from '@/views/workspacemanagement/EditWorkspaceUserRoleDialog'
...
function ShowWorkspaceRow(props) {
    ...
    const [editRoleDialogProps, setEditRoleDialogProps] = useState({})
    ...
    const handleEditWorkspaceUser = (user) => {
    ...
    const onEditRoleConfirm = () => {
        ...
        // Refresh workspace list to update user count
        if (props.onRefresh) {
            props.onRefresh()
        }
    }
        ...
        <Fragment key={props.rowKey}>
            <StyledTableRow hover sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                ...
                <StyledTableCell>
                    ...
                    <Link to={`/workspace-user-management/${props.workspace.id}`}>
                        ...
                        <Table aria-label='workspace users table'>
                            ...
                            >
                                <TableRow>
                                    ...
                                    <StyledTableCell sx={{ width: '35%' }}>Role</StyledTableCell>
                                    ...
                                    workspaceUsers.map((item, index) => (
                                        <TableRow key={index}>
                                            ...
                                            <StyledTableCell>
                                                ...
                                                ) : item.role && item.role.name === 'personal workspace' ? (
                                            ...
                                            <StyledTableCell>
                                                {!item.isOrgOwner && item.role && item.role.name !== 'personal workspace' && (
                                                    <PermissionIconButton
                                                        permissionId={'workspaces:manage'}
                                                        title='Edit Role'
                                                        color='primary'
                                                        onClick={() => handleEditWorkspaceUser(item)}
                                                    >
                                                        <IconEdit />
                                                    </PermissionIconButton>
                                                )}
            ...
            <EditWorkspaceUserRoleDialog
                ...
                dialogProps={editRoleDialogProps}
                onCancel={() => setShowEditRoleDialog(false)}
                onConfirm={onEditRoleConfirm}
            />
        </Fragment>
    )
}
...
const WorkspaceManagement = () => {
    ...
    const getAllWorkspacesApi = useApi(workspaceManagementApi.getAllWorkspacesByOrganizationId)
    const switchWorkspaceApi = useApi(workspaceManagementApi.switchWorkspace)
    ...
    const showWorkspaceUsers = (selectedWorkspace) => {
        navigate(`/workspace-user-management/${selectedWorkspace.id}`)
    }
    ...
    const deleteWorkspace = async (workspace) => {
        ...
        if (isConfirmed) {
            ...
            try {
                ...
                const deleteResp = await workspaceManagementApi.deleteWorkspace(deleteWorkspaceId, currentUser.activeOrganizationId)
                if (deleteResp.data) {
                    enqueueSnackbar({
